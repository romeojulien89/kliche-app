...
- **Optimisation post-J6, purge automatique** : `vercel.json` déclare un Cron Job quotidien (3h) sur `/api/cron/purge-guests`, protégé par `CRON_SECRET` (env var Vercel, envoyé automatiquement par Vercel Cron en `Authorization: Bearer`). Supprime chaque invité dont `purge_at` est dépassé : `DeleteFacesCommand` (Rekognition) puis la ligne `guests`. Les photos de l'événement restent (`photo_faces.guest_id` repasse à `null` via `ON DELETE SET NULL`) — seule l'identité de l'invité disparaît, conformément à la promesse de l'écran de consentement (ARTCI, 30 jours). Testé en prod avec un faux invité à `purge_at` déjà dépassé (purgé avec succès) et vérifié que l'endpoint rejette les appels sans le bon secret (401).
- **Optimisation post-J6, monitoring Sentry** : `@sentry/nextjs` branché (`src/instrumentation.ts` + `src/instrumentation-client.ts` + `sentry.server.config.ts`/`sentry.edge.config.ts` à la racine, `global-error.tsx`). Version simple, sans upload de source maps (pas de jeton Sentry supplémentaire). `Sentry.captureException()` ajouté sur les catch de `/api/photos/upload` et `/api/guests/selfie` qui correspondent à de vrais échecs inattendus (pas les cas métier attendus comme "aucun visage détecté").
  - Piège 1 : `Sentry.captureException()` seul ne suffit pas en serverless — la fonction Vercel peut se terminer avant que l'événement parte réellement sur le réseau (pas de flush automatique hors du hook `onRequestError`). Toujours faire suivre d'un `await Sentry.flush(2000)` avant que la route renvoie sa réponse (ou avant la fin d'un callback `after()`).
  - Piège 2, plus retors : le DSN (`NEXT_PUBLIC_SENTRY_DSN`) avait été ajouté sur Vercel avec le flag **"Sensitive"** coché. Les variables sensibles ne sont pas disponibles au moment du **build** (seulement à l'exécution), or `NEXT_PUBLIC_*` doit être injectée pendant le build pour être inlinée dans le bundle (client ET serveur) — le DSN se retrouvait figé à `undefined` partout, échec silencieux (aucune erreur, juste rien n'était envoyé). Diagnostiqué en comparant un envoi Sentry direct depuis un script Node local (qui arrivait bien) à un déclenchement via l'appli déployée (qui n'arrivait pas), puis en ajoutant une route de debug temporaire pour vérifier `process.env.NEXT_PUBLIC_SENTRY_DSN` en prod. Fix : recréer la variable sans "Sensitive" (un DSN n'est de toute façon pas un secret — il finit dans le bundle JS envoyé au navigateur). **Réflexe à garder** : ne jamais cocher "Sensitive" sur une variable `NEXT_PUBLIC_*`.
  - Testé de bout en bout en prod : erreur réelle déclenchée sur `/api/photos/upload`, confirmée visible dans Sentry (Issues).
- **Accès API Vercel** : un Personal Access Token Vercel (scope équipe `rocket-corp`, avec expiration) a été fourni en cours de session — stocké uniquement dans le scratchpad de session (jamais dans le repo), utilisable via `curl https://api.vercel.com/...` pour lire/modifier les env vars, lister les déploiements et suivre leur statut (`readyState`) sans capture d'écran. Team ID : `team_US32IkFCuj6vcjzPTy3lqAEG`. Le token expire — en redemander un nouveau si les appels API renvoient 401/403 dans une session future. Accélère beaucoup le diagnostic des bugs spécifiques à Vercel (toute cette section "Optimisation post-J6" a demandé des dizaines d'allers-retours par capture d'écran avant ça).
- Connu non bloquant : "livrées" dans `/studio` toujours à 0 (compteur non branché sur le matching réel, et de toute façon asynchrone maintenant), pas de notification WhatsApp Business API ni presets colorimétriques (Phase 2 du produit, hors scope V1/MVP), pas d'interface pour changer son mot de passe (à faire via Supabase Dashboard directement pour l'instant).
- Backlog du plan "puissante, efficace, dynamique, fluide" (proposé à l'utilisateur) entièrement traité : indexation en arrière-plan ✓, Realtime ✓, auth admin/photographe ✓, purge automatique ✓, monitoring d'erreurs ✓. Rien d'identifié en attente pour l'instant — prochaine étape à redéfinir avec l'utilisateur.
